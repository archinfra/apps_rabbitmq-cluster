...
RabbitMQ 这套 chart 的资源不是由安装器硬编码 `resources`，而是主要依赖 Bitnami chart 默认的 `resourcesPreset`。

当前默认情况是：

- 主 RabbitMQ 容器：`resourcesPreset = micro`
- `volumePermissions` init 容器：`resourcesPreset = nano`
- 但 `volumePermissions.enabled = false`，所以默认不会起这个 init 容器

### 默认模式资源明细

按 Bitnami `common` chart 的预设值，`micro` 大致等价于：

- request: `250m CPU / 256Mi memory / 50Mi ephemeral-storage`
- limit: `375m CPU / 384Mi memory / 2Gi ephemeral-storage`
